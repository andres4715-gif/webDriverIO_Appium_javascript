...
class Carousel extends Gestures {
    ...
    async getNthCardText(nthCard: string): Promise<string> {
        ...
        await driver.waitUntil(async () => (await this.cards.length) > 0, {
            timeoutMsg: "Expected to have more than 0 cards",
        });

        const cardNumber =
            nthCard === "first" || (await this.cards.length) === 1 ? 0 : 1;
        /**
         * IMPORTANT:
        ...
        if (driver.isAndroid) {
            const cards = await (
                await this.cards
            )[cardNumber].$$("*//android.widget.TextView");
    ...
    async getCarouselRectangles(): Promise<RectReturn> {
        ...
        CAROUSEL_RECTANGLES =
            ...
            (await driver.getElementRect(
                await $(SELECTORS.CAROUSEL).elementId
            ));
